...
export const ModalReportar = ({ codigo, onButtonClick,isDarkMode }: Props) => {
  ...
  <div>
    ...
    <div className="fixed top-0 left-0 w-screen h-screen z-10 backdrop-filter backdrop-blur-sm flex items-center justify-center">
      <div className={`p-10 rounded-lg shadow-2xl w-full max-w-2xl ${isDarkMode ? "bg-gray-800 text-white" : "bg-white text-black"}`}>
        <form onSubmit={handleSubmit}>
          <div className="mb-6">
            <div className="flex items-center justify-between w-full mb-9">
              <label className={`block text-lg font-bold mb-2 ${isDarkMode ? "text-gray-100" : "text-gray-700"}`}>
                Describe los hechos.
              </label>
            </div>
            <textarea
              className={`shadow appearance-none border rounded w-full py-3 px-4 leading-tight focus:outline-none focus:shadow-outline resize-none ${isDarkMode ? "bg-gray-700 text-gray-300 border-gray-600" : "text-gray-700"}`}
              id="descripcion"
              rows="8"
          ...
          <div className="flex items-center justify-center space-x-4">
            ...
            <button
              className={`mt-4 font-cbookF font-bold py-2 px-4 rounded-lg focus:outline-none focus:shadow-outline ${isDarkMode ? "bg-gray-600 hover:bg-gray-500 text-white" : "bg-gray-200 hover:bg-gray-300 text-cbookC-700"}`}
              type="button"
