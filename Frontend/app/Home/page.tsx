...
import { faClock, faMoon, faSun } from "@/node_modules/@fortawesome/free-solid-svg-icons/index";
...
<div className={`rounded-2xl border-2 shadow-xl col-span-8 row-span-1 mt-3 mr-3 flex items-center justify-end relative ${isDarkMode ? "bg-gray-700 border-gray-600" : "bg-white border-gray-200"}`}>
    ...
    <div className={`rounded-2xl shadow-xl row-span-3 col-span-6 flex flex-col items-center ${isDarkMode ? "bg-gray-700" : "bg-gradient-to-r from-cbookC-500 via-cbookC-700 to-cbookC-600"}`}>
 ...
 {/* Información del usuario */}
  <div className={`rounded-2xl border-2 border-gray-200 shadow-xl col-span-2 row-span-9 mr-3 mb-3 flex justify-center items-center ${isDarkMode ? 'bg-gray-700 text-white' : 'bg-white text-black'}`}>
   <div className="">
          {perfilUsuario ? (
  ...
  src={isDarkMode ? "/libro_blanco.png" : "/libro_morado.png"}
      ...
      {/* Muestra de los libros */}
          <div className={`rounded-2xl shadow-xl col-span-6 row-span-6 mb-3 overflow-auto ${isDarkMode ? "bg-gray-700" : "bg-white"}`} id="masLeidos">
 ...
 <div className="ml-4 mr-4 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 mb-4">
  {/* Mostrar hasta 4 usuarios */}
  {users.slice(0, 8).map((user) => (
    <UserCard isDarkMode={isDarkMode} key={user.codigo} {...user} />
  ))}
  {/* Insertar un separador si hay usuarios y libros */}
  {users.length > 0 && books.length > 0 && (
    <div className="col-span-full"></div>
  )}
  {/* Mostrar hasta 4 libros */}
