...
const CreateExchangeModal: React.FC<CreateExchangeModalProps> = ({ closeModal, otherUserCodigo, roomId,username,myUserCodigo,isDarkMode}) => {
...
if (!selectedBook) {
  ...
  return (
    ...
    <div className={` p-6 rounded-lg shadow-lg z-10 w-full max-w-md h-5/6 flex flex-col ${isDarkMode ? "bg-gray-700 text-white" : "bg-white text-black"}`}>
      <h2 className={`text-center font-cbookF font-bold text-3xl justify-center mt-3 mb-5 ${isDarkMode ? "text-white" : "text-cbookC-800"}`}>Crear Intercambio</h2>
        <div className="flex flex-col space-y-4">
        <label htmlFor="libro" className={`font-cbookF block ${isDarkMode ? "text-gray-300" : "text-gray-600"} font-bold mb-0 mt-1`}>Seleccionar Libro:</label>
          <select
            id="libro"
            value={selectedBook}
            onChange={(e) => setSelectedBook(e.target.value)}
          className={`rounded-lg p-2 w-full ${isDarkMode ? "bg-gray-800 text-white" : "bg-gray-100 text-gray-600"} font-cbookF focus:outline-none`}
          >
        ...
        <label htmlFor="fechaEntrega" className={`font-cbookF block ${isDarkMode ? "text-gray-300" : "text-gray-600"} font-bold mb-0 mt-1`}>Fecha y Hora de Entrega:</label>
          <input
            type="datetime-local"
            id="fechaEntrega"
...
/>
        <label htmlFor="fechaDevolucion" className={`font-cbookF block ${isDarkMode ? "text-gray-300" : "text-gray-600"} font-bold mb-0 mt-1`}>Fecha y Hora de Devolución:</label>
          <input
            type="datetime-local"
            id="fechaDevolucion"
        ...
        <label htmlFor="lugar" className={`font-cbookF block ${isDarkMode ? "text-gray-300" : "text-gray-600"} font-bold mb-0 mt-1`}>Lugar:</label>
          <input
            type="text"
            id="lugar"
            value={lugar}
...
>
            Crear Intercambio
                ...
                <button
            className={`mt-4 ${isDarkMode ? "bg-gray-600 hover:bg-gray-500 text-white" : "bg-gray-200 hover:bg-gray-300 text-cbookC-700"} font-cbookF font-bold py-2 px-4 rounded-lg focus:outline-none focus:shadow-outline`}
onClick={closeModal}        >
          Cancelar
        </button>
        </div>
